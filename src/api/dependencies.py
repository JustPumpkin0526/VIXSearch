...
from exceptions import NotFoundException, UnauthorizedException, DatabaseException
...
def get_db_cursor():
    """
    DB 커서 의존성 (컨텍스트 매니저 사용)
    
    Usage:
        @router.get("/example")
        async def example(cursor = Depends(get_db_cursor)):
...
def verify_video_ownership(
    ...
    Usage:
        ...
        async def delete_video(
            video_id: int,
            video_owner: tuple[int, str] = Depends(verify_video_ownership)
        ):
            verified_video_id, verified_user_id = video_owner
            ...
    """
    try:
        with get_db_connection() as cursor:
        ...
        logger.error(f"동영상 소유권 확인 중 오류: {e}")
        raise DatabaseException(f"동영상 소유권 확인 중 오류가 발생했습니다: {str(e)}")
