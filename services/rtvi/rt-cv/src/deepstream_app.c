...
#define RTVI_DRAIN_MAIN_DONE (1U << 0)
#define RTVI_DRAIN_FACE_DONE (1U << 1)
#define RTVI_DRAIN_REMOVE_SCHEDULED (1U << 2)

typedef struct
{
  GMutex lock;
  guint8 source_state[MAX_SOURCE_BINS];
  guint64 main_inflight[MAX_SOURCE_BINS];
  gboolean require_face_branch;
} RtviSourceDrainCoordinator;

static void
rtvi_source_drain_coordinator_free (gpointer data)
{
  RtviSourceDrainCoordinator *coordinator =
      (RtviSourceDrainCoordinator *) data;
  if (!coordinator)
    return;
...
static RtviSourceDrainCoordinator *
rtvi_source_drain_coordinator_get (AppCtx * appCtx)
{
  RtviSourceDrainCoordinator *coordinator =
      (RtviSourceDrainCoordinator *) g_object_get_data (
      G_OBJECT (appCtx->pipeline.pipeline), "rtvi-source-drain-coordinator");
  if (coordinator)
    return coordinator;

  coordinator = g_new0 (RtviSourceDrainCoordinator, 1);
  g_mutex_init (&coordinator->lock);
  coordinator->require_face_branch = rtvi_face_analytics_enabled ();
  g_object_set_data_full (G_OBJECT (appCtx->pipeline.pipeline),
      "rtvi-source-drain-coordinator", coordinator,
      rtvi_source_drain_coordinator_free);
  return coordinator;
}

static void
rtvi_remove_drained_source_async (GstElement * element, gpointer user_data)
{
  const guint source_id = GPOINTER_TO_UINT (user_data);
  GstDsNvMultiUriBin *ubin = (GstDsNvMultiUriBin *) element;
  const gboolean removed = ubin && ubin->nvmultiurisrcbinCreator &&
      gst_nvmultiurisrcbincreator_remove_source (
          ubin->nvmultiurisrcbinCreator, source_id);
  g_print ("[SOURCE_DRAIN] source=%u removal-after-drain=%s\n",
      source_id, removed ? "success" : "skipped");
}

static void
rtvi_source_drain_mark (AppCtx * appCtx, guint source_id, guint8 flag)
{
  if (!appCtx || !appCtx->pipeline.pipeline ||
      source_id >= MAX_SOURCE_BINS)
    return;

  RtviSourceDrainCoordinator *coordinator =
      rtvi_source_drain_coordinator_get (appCtx);
  gboolean schedule_remove = FALSE;

  g_mutex_lock (&coordinator->lock);
  coordinator->source_state[source_id] |= flag;
  const guint8 required = RTVI_DRAIN_MAIN_DONE |
      (coordinator->require_face_branch ? RTVI_DRAIN_FACE_DONE : 0);
  if ((coordinator->source_state[source_id] & required) == required &&
      !(coordinator->source_state[source_id] &
          RTVI_DRAIN_REMOVE_SCHEDULED)) {
    coordinator->source_state[source_id] |= RTVI_DRAIN_REMOVE_SCHEDULED;
    schedule_remove = TRUE;
  }
  g_mutex_unlock (&coordinator->lock);

  if (schedule_remove &&
      appCtx->pipeline.multi_src_bin.nvmultiurisrcbin) {
    g_print ("[SOURCE_DRAIN] source=%u all inference branches drained; "
        "scheduling removal\n", source_id);
    gst_element_call_async (
        appCtx->pipeline.multi_src_bin.nvmultiurisrcbin,
        rtvi_remove_drained_source_async, GUINT_TO_POINTER (source_id), NULL);
  }
}

static void
rtvi_source_drain_reset (AppCtx * appCtx, guint source_id)
{
  if (!appCtx || !appCtx->pipeline.pipeline ||
      source_id >= MAX_SOURCE_BINS)
    return;
  RtviSourceDrainCoordinator *coordinator =
      rtvi_source_drain_coordinator_get (appCtx);
  g_mutex_lock (&coordinator->lock);
  coordinator->source_state[source_id] = 0;
  coordinator->main_inflight[source_id] = 0;
  g_mutex_unlock (&coordinator->lock);
}

static GstPadProbeReturn
rtvi_main_branch_enter_probe (GstPad * pad, GstPadProbeInfo * info,
    gpointer user_data)
{
  (void) pad;
  ...
  RtviSourceDrainCoordinator *coordinator =
      rtvi_source_drain_coordinator_get (appCtx);
  g_mutex_lock (&coordinator->lock);
  for (NvDsMetaList *list = batch_meta->frame_meta_list; list;
      list = list->next) {
    NvDsFrameMeta *frame_meta = (NvDsFrameMeta *) list->data;
    if (frame_meta && frame_meta->source_id < MAX_SOURCE_BINS)
      coordinator->main_inflight[frame_meta->source_id]++;
  }
  g_mutex_unlock (&coordinator->lock);
  return GST_PAD_PROBE_OK;
}
...
static void
rtvi_main_branch_buffers_left (AppCtx * appCtx, NvDsBatchMeta * batch_meta)
{
  ...
  RtviSourceDrainCoordinator *coordinator =
      rtvi_source_drain_coordinator_get (appCtx);
  gboolean drained[MAX_SOURCE_BINS] = { FALSE };
  ...
  for (NvDsMetaList *list = batch_meta->frame_meta_list; list;
    ...
    if (coordinator->main_inflight[source_id] > 0)
      coordinator->main_inflight[source_id]--;
    if (coordinator->main_inflight[source_id] == 0 &&
        (coordinator->source_state[source_id] & RTVI_DRAIN_FACE_DONE) &&
        !(coordinator->source_state[source_id] & RTVI_DRAIN_MAIN_DONE))
      drained[source_id] = TRUE;
  }
  g_mutex_unlock (&coordinator->lock);

  for (guint source_id = 0; source_id < MAX_SOURCE_BINS; source_id++) {
    if (drained[source_id]) {
      g_print ("[SOURCE_DRAIN] source=%u main in-flight frames drained\n",
          source_id);
      rtvi_source_drain_mark (appCtx, source_id, RTVI_DRAIN_MAIN_DONE);
    }
  }
}

static void
rtvi_face_branch_drained (guint source_id, gpointer user_data)
{
  AppCtx *appCtx = (AppCtx *) user_data;
  if (!appCtx || source_id >= MAX_SOURCE_BINS)
    return;
  rtvi_source_drain_mark (appCtx, source_id, RTVI_DRAIN_FACE_DONE);

  RtviSourceDrainCoordinator *coordinator =
      rtvi_source_drain_coordinator_get (appCtx);
  g_mutex_lock (&coordinator->lock);
  const gboolean main_empty = coordinator->main_inflight[source_id] == 0;
  const gboolean main_pending =
      !(coordinator->source_state[source_id] & RTVI_DRAIN_MAIN_DONE);
  g_mutex_unlock (&coordinator->lock);
  if (main_empty && main_pending) {
    g_print ("[SOURCE_DRAIN] source=%u main in-flight frames drained\n",
        source_id);
    rtvi_source_drain_mark (appCtx, source_id, RTVI_DRAIN_MAIN_DONE);
  }
...
static GstPadProbeReturn
rtvi_main_branch_drain_probe (GstPad * pad, GstPadProbeInfo * info,
    gpointer user_data)
{
  (void) pad;
  if (info->type & GST_PAD_PROBE_TYPE_BUFFER) {
    GstBuffer *buffer = GST_PAD_PROBE_INFO_BUFFER (info);
    NvDsBatchMeta *batch_meta = gst_buffer_get_nvds_batch_meta (buffer);
    rtvi_main_branch_buffers_left ((AppCtx *) user_data, batch_meta);
    return GST_PAD_PROBE_OK;
  }
  if (!(info->type & GST_PAD_PROBE_TYPE_EVENT_DOWNSTREAM))
    return GST_PAD_PROBE_OK;
  GstEvent *event = GST_PAD_PROBE_INFO_EVENT (info);
  if (!event || GST_EVENT_TYPE (event) != GST_NVEVENT_STREAM_EOS)
    return GST_PAD_PROBE_OK;

  guint source_id = 0;
  gst_nvevent_parse_stream_eos (event, &source_id);
  RtviSourceDrainCoordinator *coordinator =
      rtvi_source_drain_coordinator_get ((AppCtx *) user_data);
  if (coordinator->require_face_branch)
    return GST_PAD_PROBE_OK;
  g_print ("[SOURCE_DRAIN] source=%u main-branch drained\n", source_id);
  rtvi_source_drain_mark ((AppCtx *) user_data, source_id,
      RTVI_DRAIN_MAIN_DONE);
  return GST_PAD_PROBE_OK;
}
...
bus_callback (GstBus * bus, GstMessage * message, gpointer data)
{
  ...
  switch (GST_MESSAGE_TYPE (message)) {
    ...
    case GST_MESSAGE_WARNING:{
      ...
      /* Ignore normal idle warning from dynamic streammux */
      if (g_strcmp0(GST_OBJECT_NAME(message->src), "src_bin_muxer") == 0 &&
    ...
    case GST_MESSAGE_ELEMENT:{
      ...
      if(gst_nvmessage_is_stream_add(message)) {
        ...
        rtvi_source_drain_reset (appCtx, sensorInfo.source_id);
      ...
      if(gst_nvmessage_is_stream_remove(message)) {
        ...
        rtvi_face_analytics_source_removed (appCtx->pipeline.pipeline,
    ...
    bbox_generated_callback bbox_generated_post_analytics_cb)
{
  ...
  *sink_elem = *src_elem = NULL;
  ...
  if (config->text_embedder_config.enable) {
    ...
    const gchar *text_embedder_device = g_getenv ("TEXT_EMBEDDER_DEVICE");
    if (!text_embedder_device || !text_embedder_device[0]) {
      text_embedder_device = "cuda";
    }
    g_object_set (G_OBJECT (pipeline->common_elements.text_embedder),
        "device", text_embedder_device, NULL);
    g_print ("text_embedder: device set to '%s'\n", text_embedder_device);
  ...
  if (config->tracker_config.enable) {
    GstElement *post_tracker_downstream = *sink_elem;
    ...
    if (post_tracker_downstream && rtvi_face_analytics_enabled ()) {
      GstElement *post_tracker_tee = gst_element_factory_make (
          NVDS_ELEM_TEE, "post_tracker_analytics_tee");
      GstElement *main_queue = gst_element_factory_make (
          NVDS_ELEM_QUEUE, "post_tracker_main_queue");
      if (!post_tracker_tee || !main_queue) {
        NVGSTDS_ERR_MSG_V ("Failed to create post-tracker tee/queue");
        goto done;
      }
      gst_bin_add_many (GST_BIN (pipeline->pipeline), post_tracker_tee,
          main_queue, NULL);
      NVGSTDS_LINK_ELEMENT (pipeline->common_elements.tracker_bin.bin,
          post_tracker_tee);
      if (!link_element_to_tee_src_pad (post_tracker_tee, main_queue)) {
        NVGSTDS_ERR_MSG_V ("Failed to link post-tracker main branch");
        goto done;
      }
      GstPad *main_queue_sink = gst_element_get_static_pad (main_queue,
          "sink");
      if (!main_queue_sink) {
        NVGSTDS_ERR_MSG_V ("Failed to attach main in-flight entry probe");
        goto done;
      }
      gst_pad_add_probe (main_queue_sink, GST_PAD_PROBE_TYPE_BUFFER,
          rtvi_main_branch_enter_probe, pipeline->common_elements.appCtx,
          NULL);
      gst_object_unref (main_queue_sink);
      NVGSTDS_LINK_ELEMENT (main_queue, post_tracker_downstream);
      if (!rtvi_create_face_analytics_branch (pipeline->pipeline,
              post_tracker_tee,
              config->use_nvmultiurisrcbin ? config->max_batch_size
                                           : config->primary_gie_config.batch_size,
              rtvi_face_branch_drained, pipeline->common_elements.appCtx)) {
        NVGSTDS_ERR_MSG_V ("Failed to create independent face branch");
        goto done;
      }
      GST_CAT_INFO (NVDS_APP,
          "Data flow split after tracker: main analytics and face analytics");
    } else if (post_tracker_downstream) {
      NVGSTDS_LINK_ELEMENT (pipeline->common_elements.tracker_bin.bin,
          post_tracker_downstream);
    }
  ...
  if (*src_elem) {
    /*
     * Some downstream custom elements consume GST_NVEVENT_STREAM_EOS rather
     * than forwarding it from their src pad.  Observe it on the sink pad of
     * the last common element instead: at this point every earlier PGIE/SGIE
     * buffer for the source has already left the inference chain.
     */
    GstPad *drain_pad = gst_element_get_static_pad (*src_elem, "sink");
    if (drain_pad) {
      gst_pad_add_probe (drain_pad, GST_PAD_PROBE_TYPE_BUFFER |
          GST_PAD_PROBE_TYPE_EVENT_DOWNSTREAM,
          rtvi_main_branch_drain_probe, pipeline->common_elements.appCtx,
          NULL);
      gst_object_unref (drain_pad);
    } else {
      NVGSTDS_ERR_MSG_V ("Failed to attach main inference drain probe");
      goto done;
    }
    NVGSTDS_ELEM_ADD_PROBE (pipeline->
