...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    try {
      const q = `SELECT sensor_id, filename, show_filename, timestamp, video_url, username, uploaded_at, group_id FROM uploaded_videos WHERE username = $1 ORDER BY uploaded_at DESC`;
