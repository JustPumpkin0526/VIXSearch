...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  let video_id = pickStringValue(body.video_id, body.videoId, body.id);
  let sensor_id = pickStringValue(body.sensor_id, body.sensorId);
  let filename = pickStringValue(body.filename, body.fileName, body.name);
  let storage_filename = pickStringValue(
    body.storage_filename,
    body.storageFilename,
    body.storage_name,
  );
  let video_url = pickStringValue(
    body.video_url,
    body.videoUrl,
    body.filePath,
    body.url,
  );
  let bytes = body.bytes ?? body.size ?? null;
  let timestamp = body.timestamp ?? body.created_at ?? body.createdAt ?? null;
  let uploaded_at = body.uploaded_at ?? body.uploadedAt ?? body.created_at ?? body.createdAt ?? null;
  ...
  try {
    ...
    try {
      ...
      const uploadedAtDate = toValidDate(uploaded_at, new Date());
      const timestampDate = toValidDate(timestamp, uploadedAtDate);
